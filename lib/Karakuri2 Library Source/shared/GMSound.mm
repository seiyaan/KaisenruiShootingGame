...
{
    [GMSoundImpl setListenerHorizontalOrientation:(float)radAngle];
}
...
void GMSound::setListenerPos(double x, double y, double z)
{
    ...
    sListenerPos.y = y;
    sListenerPos.z = z;
    [GMSoundImpl setListenerX:(float)x y:(float)y z:(float)z];
}
...
{
    mSourcePos = vec3;
    [(GMSoundImpl*)mSoundImpl setSourceX:(float)vec3.x y:(float)vec3.y z:(float)vec3.z];
}
